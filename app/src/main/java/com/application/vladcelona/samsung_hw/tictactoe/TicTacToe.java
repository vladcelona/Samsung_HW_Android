...
import com.application.vladcelona.samsung_hw.statistics.Statistics;
...
public class TicTacToe {
    ...
    public String result() {
        if (whoWon() > 0 ) {
            if (whoWon() == 1) {
                new Statistics().updateScore(2);
            } else {
                new Statistics().updateScore(0);
            }
            gameResult = "Player " + whoWon() + " has won";
            return "Player " + whoWon() + " has won";
        }  else if (cannotPlay()) {
            new Statistics().updateScore(1);
            gameResult = "Draw";
            return "Draw";
        } else {
